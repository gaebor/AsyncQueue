...
#define POCO_HAVE_CLOCK_GETTIME
    ...
    EventImpl::EventImpl(EventTypeImpl type) : _auto(type == EVENT_AUTORESET_IMPL), _state(false)
    {
        ...
        if (pthread_mutex_init(&_mutex, NULL))
            throw Exception("cannot create event (mutex)");
        ...
        if (pthread_condattr_init(&attr))
        {
            ...
            throw Exception("cannot create event (condition attribute)");
        }
        if (pthread_condattr_setclock(&attr, CLOCK_MONOTONIC))
        {
            pthread_condattr_destroy(&attr);
            pthread_mutex_destroy(&_mutex);
            throw Exception("cannot create event (condition attribute clock)");
        }
        if (pthread_cond_init(&_cond, &attr))
        {
            pthread_condattr_destroy(&attr);
            pthread_mutex_destroy(&_mutex);
            throw Exception("cannot create event (condition)");
        }
        pthread_condattr_destroy(&attr);
#else
    ...
    void EventImpl::waitImpl()
    {
        if (pthread_mutex_lock(&_mutex))
            throw Exception("wait for event failed (lock)");
        while (!_state)
        {
    ...
    bool EventImpl::waitImpl(long milliseconds)
    {
        ...
        while (!_state)
        {
            if ((rc = pthread_cond_timedwait(&_cond, &_mutex, &abstime)))
            {
                ...
                pthread_mutex_unlock(&_mutex);
                throw Exception("cannot wait for event");
            }
        }
